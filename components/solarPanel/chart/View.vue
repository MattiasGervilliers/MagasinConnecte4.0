...
let energyDetails = ref(props.energyDetails);

const chartKey = ref(0);

function updateChart() {
  const productionData = energyDetails.value.meters.find((meter: any) => meter.type === "Production")?.values.map((v: any) => ({ x: v.date, y: v.value ?? 0 })) || [];
  const consumptionData = energyDetails.value.meters.find((meter: any) => meter.type === "Consumption")?.values.map((v: any) => ({ x: v.date, y: v.value ?? 0 })) || [];
  const differenceData = productionData.map((v: any, i: number) => ({ x: v.x, y: (v.y ?? 0) - (consumptionData[i]?.y ?? 0) }));
